...
TEST(GraphTestSuite, VeryLargeGraphMPI){
  ...
  std::cout << "Ingesting file" << std::endl;
  for (int i = 0; i < m; ++i) {  
    in >> std::skipws >> u >> a >> b;
    if (u == static_cast<int>(UpdateType::INSERT))
      updates.push_back({{a, b}, UpdateType::INSERT});
    else
      updates.push_back({{a, b}, UpdateType::DELETE});
    if (i % 1000000 == 0)
    {
...
TEST(GraphTestSuite, VeryLargeGraphMPI_16){
  ifstream in{"/mnt/kron_16_unique_half_stream.txt"};
  Node num_nodes;
  in >> num_nodes;
  long m;
  in >> m;
...
TEST(GraphTestSuite, GenerateCum){
	write_cum("/mnt/kron_16_unique_half_stream.txt","/mnt/kron_16_graph.txt");
}
