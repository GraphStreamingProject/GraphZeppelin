...
TEST(SketchTestSuite, TestSketchSizeEstimate) {
  size_t n = 4096;
  size_t s = 26;
  size_t dense_est = DenseSketch::estimate_bytes(n, s);
  size_t sparse_est = SparseSketch::estimate_bytes(n, s);

  DenseSketch ds(n, 0, s);
  SparseSketch ss(n, 0, s);

  ASSERT_EQ(ds.bucket_array_bytes(), dense_est);
