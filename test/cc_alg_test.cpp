...
TEST(CCAlgTest, MultipleSpanningForests) {
  auto driver_config = DriverConfiguration().gutter_sys(STANDALONE);
  auto cc_config = CCAlgConfiguration().sketches_factor(1.6);
  generate_stream(get_seed(), 1024, 0.1, 0.1, 0.05, 3, "sample.txt", "cumul_sample.txt");
  AsciiFileStream stream{"./sample.txt"};
  node_id_t num_nodes = stream.vertices();
  ...
  // do a bunch of query calls to ensure sketches are in a good state after queries.
  cc_alg.calc_disjoint_spanning_forests(8);
