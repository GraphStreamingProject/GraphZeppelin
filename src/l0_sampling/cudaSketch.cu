...
__global__ void sketch_update(vec_t* bucket_a, vec_hash_t* bucket_c, const size_t num_elems, const size_t num_buckets, const size_t num_guesses, const uint64_t seed, 
                              col_hash_t* col_index_hash, const vec_t update_idx, const vec_hash_t update_hash) {
...
void CudaSketch::update(col_hash_t* d_col_index_hash, const vec_t& update_idx) {
  ...
  sketch_update<<<num_blocks, num_threads>>>(bucket_a, bucket_c, num_elems, num_buckets, num_guesses, seed, d_col_index_hash, update_idx, update_hash);
}