...
namespace {
constexpr uintptr_t capacity_mask = 0xFFu;
constexpr std::align_val_t tagged_alignment = std::align_val_t(256);

Bucket* allocate_bucket_block_impl(uint8_t capacity) {
  const size_t count = static_cast<size_t>(capacity) + 1;
  auto* raw = static_cast<Bucket*>(
      ::operator new[](count * sizeof(Bucket), tagged_alignment));
  std::memset(raw, 0, count * sizeof(Bucket));
  return raw;
}

void free_bucket_block_impl(Bucket* raw) {
  ::operator delete[](raw, tagged_alignment);
}
}  // namespace

void FixedSizeSketchColumn::set_tagged_buckets(Bucket* raw_buckets,
                                               uint8_t capacity) {
  assert((reinterpret_cast<uintptr_t>(raw_buckets) & capacity_mask) == 0);
...
FixedSizeSketchColumn::FixedSizeSketchColumn(const FixedSizeSketchColumn& other)
  ...
  if (other.is_initialized()) {
    set_tagged_buckets(allocate_bucket_block(other.capacity()),
                       other.capacity());
...
FixedSizeSketchColumn::FixedSizeSketchColumn(
    FixedSizeSketchColumn&& other) noexcept
    : buckets_tagged(other.buckets_tagged), seed(other.seed) {
  other.buckets_tagged = 0;
}

FixedSizeSketchColumn& FixedSizeSketchColumn::operator=(
    FixedSizeSketchColumn&& other) noexcept {
  if (this != &other) {
...
FixedSizeSketchColumn& FixedSizeSketchColumn::operator=(
    const FixedSizeSketchColumn& other) {
  if (this != &other) {
    free_bucket_block(buckets_raw());
    buckets_tagged = 0;
    seed = other.seed;
...
uint8_t FixedSizeSketchColumn::get_depth() const {
  for (size_t i = capacity(); i > 0; --i) {
    if (!Bucket_Boruvka::is_empty(sketch_buckets()[i - 1])) {
      return i;
    }
  }
  return 0;
...
size_t FixedSizeSketchColumn::space_usage_bytes() const {
  return sizeof(FixedSizeSketchColumn) +
         ((static_cast<size_t>(capacity()) + 1) * sizeof(Bucket));
}

void FixedSizeSketchColumn::serialize(std::ostream& binary_out) const {
  binary_out.write((char*)sketch_buckets(), capacity() * sizeof(Bucket));
...
SketchSample<vec_t> FixedSizeSketchColumn::sample() const {
  ...
  for (size_t i = capacity(); i > 0; --i) {
    if (Bucket_Boruvka::is_good(sketch_buckets()[i - 1], seed)) {
      return {sketch_buckets()[i - 1].alpha, GOOD};
    }
  }
  return {0, FAIL};
}
...
void FixedSizeSketchColumn::prefetch() {
  ...
  size_t num_lines =
      (capacity() * sizeof(Bucket) + cache_line_size - 1) / cache_line_size;
  for (size_t i = 0; i < num_lines; ++i) {
    _mm_prefetch(
        reinterpret_cast<const char*>(sketch_buckets()) + i * cache_line_size,
        _MM_HINT_T0);
  }
}
...
void FixedSizeSketchColumn::merge(FixedSizeSketchColumn const& other) {
  for (size_t i = 0; i < capacity(); ++i) {
    sketch_buckets()[i] ^= other.sketch_buckets()[i];
  }
...
void FixedSizeSketchColumn::update(const vec_t update) {
  ...
  col_hash_t depth =
      Bucket_Boruvka::get_index_depth_legacy(update, seed, capacity() - 1);
  sketch_buckets()[depth] ^= {update, checksum};
  deterministic_bucket_ref() ^= {update, checksum};
}

const ColumnEntryDelta FixedSizeSketchColumn::generate_entry_delta(
    vec_t update) const {
  vec_hash_t checksum = Bucket_Boruvka::get_index_hash(update, seed);
  col_hash_t depth =
      Bucket_Boruvka::get_index_depth_legacy(update, seed, capacity() - 1);
...
void FixedSizeSketchColumn::apply_entry_delta(const ColumnEntryDelta& delta) {
  ...
  sketch_buckets()[delta.depth] ^= delta.bucket;
...
void FixedSizeSketchColumn::atomic_update(const vec_t update) {
  ...
  std::atomic_ref<vec_t> bucket_alpha(sketch_buckets()[depth].alpha);
  std::atomic_ref<vec_hash_t> bucket_gamma(sketch_buckets()[depth].gamma);
...
void FixedSizeSketchColumn::atomic_apply_entry_delta(
    const ColumnEntryDelta& delta) {
  assert(delta.depth < capacity());

  std::atomic_ref<vec_t> det_alpha(deterministic_bucket_ref().alpha);
  std::atomic_ref<vec_hash_t> det_gamma(deterministic_bucket_ref().gamma);

  std::atomic_ref<vec_t> bucket_alpha(sketch_buckets()[delta.depth].alpha);
  std::atomic_ref<vec_hash_t> bucket_gamma(sketch_buckets()[delta.depth].gamma);
...
void ResizeableSketchColumn::set_tagged_buckets(Bucket* raw_buckets,
                                                uint8_t capacity) {
  assert((reinterpret_cast<uintptr_t>(raw_buckets) & capacity_mask) == 0);
  buckets_tagged = reinterpret_cast<uintptr_t>(raw_buckets) | capacity;
}
...
ResizeableSketchColumn::ResizeableSketchColumn(
    const ResizeableSketchColumn& other)
    : seed(other.seed) {
  if (other.is_initialized()) {
    set_tagged_buckets(allocate_bucket_block(other.capacity()),
                       other.capacity());
...
ResizeableSketchColumn::ResizeableSketchColumn(
    ResizeableSketchColumn&& other) noexcept
    : buckets_tagged(other.buckets_tagged), seed(other.seed) {
  other.buckets_tagged = 0;
}

ResizeableSketchColumn& ResizeableSketchColumn::operator=(
    ResizeableSketchColumn&& other) noexcept {
  if (this != &other) {
    free_bucket_block(buckets_raw());
    buckets_tagged = other.buckets_tagged;
...
void ResizeableSketchColumn::prefetch() {
  ...
  size_t num_lines =
      (capacity() * sizeof(Bucket) + cache_line - 1) / cache_line;
  for (size_t i = 0; i < num_lines; ++i) {
    _mm_prefetch(
        reinterpret_cast<const char*>(sketch_buckets()) + (i * cache_line),
        _MM_HINT_T0);
  }
}
...
const ColumnEntryDelta ResizeableSketchColumn::generate_entry_delta(
    vec_t update) const {
  vec_hash_t checksum = Bucket_Boruvka::get_index_hash(update, seed);
  col_hash_t depth = Bucket_Boruvka::get_index_depth_legacy(update, seed, 60);
  return {Bucket{update, checksum}, static_cast<uint16_t>(depth)};
...
void ResizeableSketchColumn::atomic_update(const vec_t update) {
  ...
  if (depth < capacity()) {
    ...
    this->lock.unlock_shared();
    this->lock.lock();
    size_t desired_capacity = std::max(static_cast<size_t>(depth + 1),
...
void ResizeableSketchColumn::atomic_apply_entry_delta(
    const ColumnEntryDelta& delta) {
  this->lock.lock_shared();
  if (delta.depth < capacity()) {
    std::atomic_ref<vec_t> det_alpha(deterministic_bucket_ref().alpha);
    std::atomic_ref<vec_hash_t> det_gamma(deterministic_bucket_ref().gamma);
    std::atomic_ref<vec_t> bucket_alpha(sketch_buckets()[delta.depth].alpha);
    std::atomic_ref<vec_hash_t> bucket_gamma(
        sketch_buckets()[delta.depth].gamma);
    det_alpha.fetch_xor(delta.bucket.alpha, std::memory_order_relaxed);
    det_gamma.fetch_xor(delta.bucket.gamma, std::memory_order_relaxed);
    bucket_alpha.fetch_xor(delta.bucket.alpha, std::memory_order_relaxed);
    bucket_gamma.fetch_xor(delta.bucket.gamma, std::memory_order_relaxed);
    ...
    this->lock.unlock_shared();
    this->lock.lock();
    size_t desired_capacity = std::max(static_cast<size_t>(delta.depth + 1),
                                       static_cast<size_t>(capacity()));
    if (desired_capacity != capacity()) {
      reallocate(static_cast<uint8_t>(desired_capacity));
    }
...
size_t ResizeableSketchColumn::space_usage_bytes() const {
  return sizeof(ResizeableSketchColumn) +
         ((static_cast<size_t>(capacity()) + 1) * sizeof(Bucket));
}

void ResizeableAlignedSketchColumn::set_tagged_buckets(Bucket* raw_buckets,
                                                       uint8_t capacity) {
  assert((reinterpret_cast<uintptr_t>(raw_buckets) & capacity_mask) == 0);
  buckets_tagged = reinterpret_cast<uintptr_t>(raw_buckets) | capacity;
}
...
ResizeableAlignedSketchColumn::ResizeableAlignedSketchColumn(
    uint8_t start_capacity, uint64_t seed)
    : seed(seed) {
  if (start_capacity > 0) {
    set_tagged_buckets(allocate_bucket_block(start_capacity), start_capacity);
  }
...
ResizeableAlignedSketchColumn& ResizeableAlignedSketchColumn::operator=(
    ResizeableAlignedSketchColumn&& other) noexcept {
  if (this != &other) {
    free_bucket_block(buckets_raw());
    buckets_tagged = other.buckets_tagged;
...
const ColumnEntryDelta ResizeableAlignedSketchColumn::generate_entry_delta(
    vec_t update) const {
  vec_hash_t checksum = Bucket_Boruvka::get_index_hash(update, seed);
  col_hash_t depth = Bucket_Boruvka::get_index_depth_legacy(update, seed, 60);
  return {Bucket{update, checksum}, static_cast<uint16_t>(depth)};
...
void ResizeableAlignedSketchColumn::apply_entry_delta(
    const ColumnEntryDelta& delta) {
  deterministic_bucket_ref() ^= delta.bucket;
...
void ResizeableAlignedSketchColumn::merge(
    ResizeableAlignedSketchColumn const& other) {
  deterministic_bucket_ref() ^= other.deterministic_bucket_ref();
  if (other.capacity() > capacity()) {
    reallocate(other.capacity());
  }
  uint32_t* for_vector_merge = reinterpret_cast<uint32_t*>(sketch_buckets());
  const uint32_t* other_for_vector_merge =
      reinterpret_cast<const uint32_t*>(other.sketch_buckets());
  int num_vectors = other.capacity() * (sizeof(Bucket) / sizeof(uint32_t));
  hwy::HWY_NAMESPACE::simd_xor(for_vector_merge, other_for_vector_merge,
                               num_vectors);
}
...
size_t ResizeableAlignedSketchColumn::space_usage_bytes() const {
  return sizeof(ResizeableAlignedSketchColumn) +
         ((static_cast<size_t>(capacity()) + 1) * sizeof(Bucket));
}
...
static_assert(
    SketchColumnConcept<ResizeableAlignedSketchColumn, vec_t>,
