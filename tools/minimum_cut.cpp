 ...
 */
static void track_insertions(uint64_t total, GraphSketchDriver<MinCutSketchAlg> *driver,
...
int main(int argc, char **argv) {
  ...
  for (size_t q = 0; q < queries_in_stream; q++) {
    ...
    auto mc = mc_alg.calc_minimum_cut();
    std::chrono::duration<double> mc_time = std::chrono::steady_clock::now() - query_start;
    std::chrono::duration<double> flush_time = driver.flush_end - driver.flush_start;

    std::cout << "Query " << q + 1 << std::endl;
    std::cout << "Total Minimum cut latency:  " << mc_time.count() << std::endl;
    std::cout << "  Flush Gutters(sec):         " << flush_time.count() << std::endl;
    std::cout << "  Creating SFs(sec):          " << mc_alg.sf_total_duration.count() << std::endl;
    std::cout << "  Viecut algorithm(sec):      " << mc_alg.viecut_duration.count() << std::endl;
    std::cout << "Minimum Cut:                " << mc.value << std::endl;
