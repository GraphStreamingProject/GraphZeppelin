...
class CudaGraph {
    public: 
        ...
        bool isInit = false; 
        bool kInit = false;
        bool indiv_graph_enabled = false;
        int indiv_graph_id = -1;
        ...
        void k_batch_update(int id, node_id_t src, const std::vector<node_id_t> &edges) {
            ...
            while(true) {
                if (cudaStreamQuery(streams[stream_id + stream_offset]) == cudaSuccess) {
                    ...
                    if(streams_deltaApplied[stream_id] == 0) {
                        ...
                        if (indiv_graph_enabled) {
                            if (indiv_graph_id == -1) {
                                std::cout << "ERROR: CudaGraph: indiv_graph_id is -1!\n";
                            }
                            cudaMemcpyAsync(&cudaUpdateParams[indiv_graph_id]->h_bucket_a[k * stream_id * sketch_size], &cudaUpdateParams[indiv_graph_id]->d_bucket_a[k * stream_id * sketch_size], k * sketch_size * sizeof(vec_t), cudaMemcpyDeviceToHost, streams[stream_id]);
                            cudaMemcpyAsync(&cudaUpdateParams[indiv_graph_id]->h_bucket_c[k * stream_id * sketch_size], &cudaUpdateParams[indiv_graph_id]->d_bucket_c[k * stream_id * sketch_size], k * sketch_size * sizeof(vec_hash_t), cudaMemcpyDeviceToHost, streams[stream_id]);
                        }
                        else {
                        ...
                        if (indiv_graph_enabled) {
                            ...
                            Supernode** graph_supernodes = all_supernodes[indiv_graph_id];
                            for (int i = 0; i < k; i++) {
                                for (int j = 0; j < cudaUpdateParams[0]->num_sketches; j++) {
                                    ...
                                    for (size_t m = 0; m < cudaUpdateParams[0]->num_elems; m++) {
                                        bucket_a[m] ^= cudaUpdateParams[indiv_graph_id]->h_bucket_a[(stream_id * sketch_size * k) + (i * sketch_size) + (j * cudaUpdateParams[0]->num_elems) + m];
                                        bucket_c[m] ^= cudaUpdateParams[indiv_graph_id]->h_bucket_c[(stream_id * sketch_size * k) + (i * sketch_size) + (j * cudaUpdateParams[0]->num_elems) + m];
                                    }
                                }
                            }
                        }
            ...
            if (indiv_graph_enabled) {
                ...
                for (size_t i = 0; i < edges.size(); i++) {
                    ...
                    cudaUpdateParams[indiv_graph_id]->h_edgeUpdates[start_index + edge_count[indiv_graph_id]] = edge_id;
                    edge_count[indiv_graph_id]++;
                }
                streams_num_graphs[stream_id] = 0;

                std::unique_lock<std::mutex> graph_lk(graph_mutexes[indiv_graph_id]);
                cudaUpdateParams[indiv_graph_id]->num_inserted_updates += edge_count[indiv_graph_id];
                graph_lk.unlock();
                ...
                cudaMemcpyAsync(&cudaUpdateParams[indiv_graph_id]->d_edgeUpdates[start_index], &cudaUpdateParams[indiv_graph_id]->h_edgeUpdates[start_index], edge_count[indiv_graph_id] * sizeof(vec_t), cudaMemcpyHostToDevice, streams[stream_id]);
                cudaKernel.k_gtsStreamUpdate(num_device_threads, num_device_blocks, indiv_graph_id, k, (stream_id * sketch_size * k), src, streams[stream_id], start_index, edge_count[indiv_graph_id], cudaUpdateParams[indiv_graph_id], &sketchSeeds[indiv_graph_id * num_nodes * num_sketches * k]);
            }
            else {
                for (size_t i = 0; i < edges.size(); i++) {
                ...
                for (int graph_id = 0; graph_id <= streams_num_graphs[stream_id]; graph_id++) {
                    std::unique_lock<std::mutex> graph_lk(graph_mutexes[graph_id]);
                    cudaUpdateParams[graph_id]->num_inserted_updates += edge_count[graph_id];
                    graph_lk.unlock();
                }
                ...
                for (int graph_id = 0; graph_id <= streams_num_graphs[stream_id]; graph_id++) {
                    ...
                    /*if (graph_id != streams_num_graphs[stream_id] - 1) {
                        cudaStreamSynchronize(streams[stream_id]);
                    }*/
                }
            }
        ...
        void batch_update(int id, node_id_t src, const std::vector<node_id_t> &edges) {
            ...
            cudaKernel.gtsStreamUpdate(num_device_threads, num_device_blocks, stream_id * sketch_size, src, streams[stream_id], start_index, edges.size(), cudaUpdateParams[0], sketchSeeds);
        };
            ...
            for (int stream_id = 0; stream_id < num_host_threads * stream_multiplier; stream_id++) {
                ...
                if(streams_deltaApplied[stream_id] == 0) {
                    ...
                    if (indiv_graph_enabled) {
                        ...
                        // Bring back delta sketches
                        cudaMemcpy(&cudaUpdateParams[indiv_graph_id]->h_bucket_a[k * stream_id * sketch_size], &cudaUpdateParams[indiv_graph_id]->d_bucket_a[k * stream_id * sketch_size], k * sketch_size * sizeof(vec_t), cudaMemcpyDeviceToHost);
                        cudaMemcpy(&cudaUpdateParams[indiv_graph_id]->h_bucket_c[k * stream_id * sketch_size], &cudaUpdateParams[indiv_graph_id]->d_bucket_c[k * stream_id * sketch_size], k * sketch_size * sizeof(vec_hash_t), cudaMemcpyDeviceToHost);
                        // Apply the delta sketch
