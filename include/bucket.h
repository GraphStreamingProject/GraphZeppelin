...
namespace Bucket_Boruvka {
   ...
   */
  inline static col_hash_t col_index_hash(const vec_t update_idx, const
  uint64_t seed_and_col);
   ...
   */
  inline static vec_hash_t index_hash(const vec_t index, uint64_t seed);
   ...
   */
  inline static bool is_good(const vec_t a, const vec_hash_t c, const
   ...
   */
  inline static bool is_good(const vec_t a, const vec_hash_t c, const
  unsigned bucket_col, const vec_t guess_nonzero, const uint64_t sketch_seed);
...
inline col_hash_t Bucket_Boruvka::col_index_hash(const vec_t update_idx,
                                                 const uint64_t seed_and_col) {
  return col_hash(update_idx, seed_and_col);
}

inline vec_hash_t Bucket_Boruvka::index_hash(const vec_t index, uint64_t
sketch_seed) {
  return vec_hash(index, sketch_seed);
}
...
inline bool Bucket_Boruvka::is_good(const vec_t a, const vec_hash_t c,
                                    const uint64_t sketch_seed) {
  return c == index_hash(a, sketch_seed);
}

inline bool Bucket_Boruvka::is_good(const vec_t a, const vec_hash_t c,
                                    const unsigned bucket_col, const vec_t
                                    guess_nonzero, const uint64_t sketch_seed) {
  return c == index_hash(a, sketch_seed)
