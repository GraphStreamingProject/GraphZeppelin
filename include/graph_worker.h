...
class GraphWorker {
public:
	/* start_workers creates the graph workers and sets them to query the database given.
	 * @param _graph   the graph to update
	 * @param _db/_wq  the database to query
	 * @param _supernode_size  the size of a supernode so that we can allocate space for a delta_node
	 */
#ifdef USE_FBT_F
